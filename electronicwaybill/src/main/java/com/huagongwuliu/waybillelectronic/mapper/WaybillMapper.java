...
//    @Select("select * from tb_eway where goods_name LIKE concat('%',#{goodsName},'%') and user_id = #{userId}  and goods_status = 0 order by add_time desc")
    ...
    @Select( "<script>" +
            " select e.* from tb_eway e" +
            "<if test=' userPhone != null '>  LEFT JOIN app_user_info u ON e.user_id = u.id  </if>" +
            "where  1=1  " +
            "<if test=' userPhone != null '>  and u.phone like concat('%',#{userPhone},'%')  </if>" +
            "<if test=' waybill.shipperName != null '>and e.shipper_name like concat('%',#{waybill.shipperName},'%') </if>" +
            "<if test=' waybill.shipperContact != null '>and e.shipper_contact like concat('%',#{waybill.shipperContact},'%') </if>" +
            "<if test=' waybill.shipperPhone != null '>and e.shipper_phone like concat('%',#{waybill.shipperPhone},'%') </if>" +

            "<if test=' waybill.shiptoName != null '>and e.shipto_name like concat('%',#{waybill.shiptoName},'%') </if>" +
            "<if test=' waybill.shiptoPhone != null '>and e.shipto_phone like concat('%',#{waybill.shiptoPhone},'%') </if>" +

            "<if test=' waybill.shipmentName != null '>and e.shipment_name like concat('%',#{waybill.shipmentName},'%') </if>" +
            "<if test=' waybill.shipmentPhone != null '>and e.shipment_phone like concat('%',#{waybill.shipmentPhone},'%') </if>" +
            "<if test=' waybill.shipmentStarttime != null '>and e.shipment_starttime like concat('%',#{waybill.shipmentStarttime},'%') </if>" +
            "<if test=' waybill.shipmentFromAddress != null '>and e.shipment_from_address like concat('%',#{waybill.shipmentFromAddress},'%') </if>" +
            "<if test=' waybill.shipmentFromDetails != null '>and e.shipment_from_details like concat('%',#{waybill.shipmentFromDetails},'%') </if>" +
            "<if test=' waybill.cityExpress != null '>and e.city_express like concat('%',#{waybill.cityExpress},'%') </if>" +
            "<if test=' waybill.shipmentToAddress != null '>and e.shipment_to_address like concat('%',#{waybill.shipmentToAddress},'%') </if>" +
            "<if test=' waybill.shipmentToDetails != null '>and e.shipment_to_details like concat('%',#{waybill.shipmentToDetails},'%') </if>" +

            "<if test=' waybill.carriageName != null '>and e.carriage_name like concat('%',#{waybill.carriageName},'%') </if>" +
            "<if test=' waybill.carriagePhone != null '>and e.carriage_phone like concat('%',#{waybill.carriagePhone},'%') </if>" +
            "<if test=' waybill.carriageLicensekey != null '>and e.carriage_licensekey like concat('%',#{waybill.carriageLicensekey},'%') </if>" +

            "<if test=' waybill.licensePlateNum != null '>and e.license_plate_num like concat('%',#{waybill.licensePlateNum},'%') </if>" +
            "<if test=' waybill.licensePlateColor != null '>and e.license_plate_color like concat('%',#{waybill.licensePlateColor},'%') </if>" +
            "<if test=' waybill.roadTransportPermitNum != null '>and e.road_transport_permit_num like concat('%',#{waybill.roadTransportPermitNum},'%') </if>" +
            "<if test=' waybill.trailerRoadRansportPermit != null '>and e.trailer_road_ransport_permit like concat('%',#{waybill.trailerRoadRansportPermit},'%') </if>" +
            "<if test=' waybill.canbodyNum != null '>and e.canbody_num like concat('%',#{waybill.canbodyNum},'%') </if>" +
            "<if test=' waybill.driverName != null '>and e.driver_name like concat('%',#{waybill.driverName},'%') </if>" +
            "<if test=' waybill.driverCertificate != null '>and e.driver_certificate like concat('%',#{waybill.driverCertificate},'%') </if>" +
            "<if test=' waybill.driverPhone != null '>and e.driver_phone like concat('%',#{waybill.driverPhone},'%') </if>" +
            "<if test=' waybill.escortName != null '>and e.escort_name like concat('%',#{waybill.escortName},'%') </if>" +
            "<if test=' waybill.escortPhone != null '>and e.escort_phone like concat('%',#{waybill.escortPhone},'%') </if>" +
            "<if test=' waybill.escortCertificate != null '>and e.escort_certificate like concat('%',#{waybill.escortCertificate},'%') </if>" +

            "<if test=' waybill.goodsName != null '>and e.goods_name like concat('%',#{waybill.goodsName},'%') </if>" +
            "<if test=' waybill.unNum != null '>and e.un_num like concat('%',#{waybill.unNum},'%') </if>" +
            "<if test=' waybill.goodsType != null '>and e.goods_type like concat('%',#{waybill.goodsType},'%') </if>" +
            "<if test=' waybill.goodsPackingNorms != null '>and e.goods_packing_norms like concat('%',#{waybill.goodsPackingNorms},'%') </if>" +

            "<if test=' waybill.goodsPackingType != null '>and e.goods_packing_type like concat('%',#{waybill.goodsPackingType},'%') </if>" +
            "<if test=' waybill.goodsNum != null '>and e.goods_num like concat('%',#{waybill.goodsNum},'%') </if>" +
            "<if test=' waybill.goodsCompany != null '>and e.goods_company like concat('%',#{waybill.goodsCompany},'%') </if>" +

            "<if test=' waybill.dispatcherName != null '>and e.dispatcher_name like concat('%',#{waybill.dispatcherName},'%') </if>" +
            "<if test=' waybill.dispatcherDate != null '>and e.dispatcher_date like concat('%',#{waybill.dispatcherDate},'%') </if>" +
            "<if test=' waybill.remarks != null '>and e.remarks like concat('%',#{waybill.remarks},'%') </if>" +
            ...
            "</script>")
      List<Waybill> queryallbackstage(Waybill waybill,Long startTime, Long endTime, Boolean desc,String userPhone);



//    "<if test=' waybill.shiptoPhone != null '>and (shipto_phone like concat('%',#{waybill.shiptoPhone},'%') " +
//            "or shipment_phone like concat('%',#{waybill.shipmentPhone},'%') " +
//            "or carriage_phone like concat('%',#{waybill.carriagePhone},'%') " +
//            "or driver_phone like concat('%',#{waybill.driverPhone},'%') " +
//            "or  escort_phone like concat('%',#{waybill.escortPhone},'%') " +
//            ") </if>" +
